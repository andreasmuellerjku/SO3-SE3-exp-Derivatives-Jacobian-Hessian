...
SE3PDdexp::usage = "SE3PDdexp[X, U, eps] returns the derivative D(dexp)_X[U] on se(3).";
SE3PdexpInv::usage = "SE3PdexpInv[X, eps] returns the inverse of dexp at twist X.";
SE3PDdexpInv::usage = "SE3PDdexpInv[X, U, eps] returns the derivative of dexp^{-1} at X applied to U.";
...
SE3PExp[X_, eps_:SE3EPS] := Module[{x, y, R, p, n, a, b, nx}, 
   ...
   b = Sinc[n/2]^2; 
   nx = x/n;
   R = IdentityMatrix[3] + a*AxisToso3[x] + (b/2)*AxisToso3[x] . AxisToso3[x];
   p = ((b/2)*AxisToso3[x] - a*AxisToso3[nx] . AxisToso3[nx] + Transpose[{nx}] . {nx}) . y;
