...
Reference: [2] eq. 2.8 *)
SO3Exp[x_?VectorQ] := IdentityMatrix[3] + Sinc[Sqrt[x . x]]*AxisToso3[x] + 
       (Sinc[Sqrt[x . x]/2]^2/2)*AxisToso3[x] . AxisToso3[x]
Protect[SO3Exp];
...
SO3Log[R_, eps_:SO3EPS] := Module[{t}, 
    ...
    Return[t*(so3ToAxis[R - Transpose[R]]/(2*Sin[t]))]; ]
Protect[SO3Log]; 
...
SO3Ddexp[x_,y_,eps_:SO3EPS]:=Module[{n,n2, nx, xtilde, nxtilde, ytilde, Alpha, Beta, Delta},
  ...
  n = Sqrt[n2];
  ytilde = AxisToso3[y];
  If[n<eps, Return[1/2*AxisToso3[y]]];
  Alpha =Sinc[n];
  Beta =(Sinc[n/2])^2;
  Delta =(1-Alpha)/n2;
  xtilde = AxisToso3[x];
  nx = x/n;
  nxtilde = AxisToso3[nx];
